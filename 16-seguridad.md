...
- Sin cesión a terceros para fines de marketing/redes sociales: el sistema no exporta, comparte ni sincroniza datos de punteo con ningún servicio externo de ese tipo. **Excepción explícita, decidida por Gaspar el 2026-08-03**: el contenido de `PunteoComentario` sí se envía a la API de Gemini cuando el chatbot de IA (Fase 9, [`15-ia.md`](./15-ia.md#7-chatbot-conectado-a-la-base-de-datos)) responde una pregunta sobre el punteo de una persona puntual — dentro de exactamente el mismo alcance de permisos que la UI (un usuario sin `punteo.ver_todos` solo puede disparar el envío de sus propios comentarios; ver el resto de esta sección para el aislamiento por usuario y la auditoría de lectura, que siguen aplicando igual). Esta excepción puntual sí constituye cesión de dato sensible a un tercero (Google) y queda documentada acá porque contradice la caracterización general de este párrafo — no la reabre como pregunta abierta, es una decisión ya tomada.
...
- El **chatbot** solo accede a datos dentro del alcance de permisos del usuario que pregunta (ver [`15-ia.md`](./15-ia.md#73-el-chatbot-respeta-los-mismos-permisos-que-el-resto-del-sistema)), nunca a la base completa sin ese filtro. **Esto incluye el contenido de `PunteoComentario`** (decisión de Gaspar, 2026-08-03, ver la excepción documentada en la sección 6): cuando el usuario que pregunta tiene acceso a un comentario de punteo puntual (el suyo propio siempre; el de otros usuarios solo con `punteo.ver_todos`, auditado igual que en la UI), ese texto se envía a Gemini como parte de la respuesta. Es la única funcionalidad de IA del sistema que envía dato sensible (afinidad política) a un proveedor externo — evaluar puntualmente si esto requiere ajustar el checklist legal de la sección 13 antes de operar con datos reales de punteo a gran escala.
- Se recomienda revisar los términos de tratamiento de datos y retención de la API de Gemini vigentes al momento de la implementación, como parte del checklist de la sección 13 — con prioridad especial dado el punto anterior.
