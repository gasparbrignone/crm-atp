...
│   │   ├── deteccion-duplicados.ts normalizacion.ts matching-padron.ts chatbot.ts insights.ts
│   ├── identidad/                  # Motor de Resolución de Identidad — determinístico, sin IA (ver su README.md)
│   │   ├── algoritmos.ts normalizar.ts motor-scoring.ts resolucion.ts politica-decision.ts
│   ├── padron/                     # Lectura de padrones PDF — determinística, sin IA desde 2026-08-04 (ver lib/padron/lectura-padron.ts)
│   │   ├── lectura-padron.ts
...
**Cambio de estado (2026-08-04): primer dato real de un tercero cargado — el checklist de seguridad pasa a ser una restricción activa.** Hasta esta fecha, el único dato personal en la base era el del propio Gaspar, para pruebas ("fase de construcción, sin datos reales de terceros"). El 2026-08-04 se completó la carga del padrón electoral real de Consejo Directivo de Medicina (5356 personas reales, nombre y DNI) — al arreglar el importador de padrón (ver `15-ia.md` sección 4 y `lib/padron/lectura-padron.ts`), que había quedado bloqueado por un corte de acceso del proveedor de IA a mitad de una carga real ya iniciada por Gaspar. Por regla ya establecida en esta misma sección desde antes: **el checklist de la sección 13 de `16-seguridad.md` (RLS real, MFA, backups verificados, revisión legal del módulo de punteo, rotación de credenciales, sin secretos en el repo, auditoría de acceso a punteo ajeno, revisión de términos de IA) deja de ser una condición de salida futura y pasa a ser una restricción activa sobre el proyecto desde este momento.** Ninguno de los 8 ítems de ese checklist está resuelto todavía (verificado 2026-08-04) — es una prioridad real pendiente de que Gaspar decida cómo abordarla, no una tarea que una sesión de Claude Code deba iniciar por su cuenta sin que él lo pida explícitamente (implica decisiones legales y de infraestructura fuera del alcance de escribir código). Ver el análisis completo del contexto anterior en `REVISION-CRITICA-AUDITORIA-2026-08-04.md` sección 0 (nota: ese documento describe la fase *anterior* a este cambio, hay que leerlo con esa fecha de corte en mente).
...
**Plan de Vercel: siempre el gratuito (Hobby)** — decisión explícita de Gaspar (2026-08-02): este proyecto no paga por infraestructura. No asumir el límite de duración de función sin confirmarlo contra la documentación oficial de Vercel (bug real, dos veces el mismo día: primero se asumió por error que el proyecto estaba en plan Pro con funciones de hasta 800s; después se sobrecorrigió a 60s, mucho más bajo del real). El número real confirmado (Hobby, con Fluid Compute — activado por defecto): **300s de default y de máximo**, no configurable más alto en este plan. Cualquier tarea que pueda tardar más que eso (como la carga de padrones, ver `15-ia.md` sección 10 y `09-modulo-padron-electoral.md`) tiene que diseñarse como procesamiento incremental en pasos cortos, no como una sola función de larga duración — ya no por cuota de IA (la lectura de padrón es determinística desde el 2026-08-04), sino por el volumen real de consultas a la base durante el matching de miles de filas.
