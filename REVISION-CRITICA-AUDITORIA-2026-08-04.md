...
4. **"Lista de espera" en Actividades está documentada como disparador de notificación (`13-notificaciones.md` sección 3: "se liberó un cupo... la siguiente persona en la lista de espera") pero el modelo de datos no tiene ningún estado de espera** (`EstadoParticipacion` solo tiene `inscripto/confirmado/asistio/ausente/cancelado`, sin `en_espera`). Lo detecté al implementar Fase 11 esta misma sesión y tuve que omitir ese disparador específico por no tener dónde engancharlo. Es una funcionalidad documentada en dos módulos distintos (Participaciones y Notificaciones) que nunca se construyó en ninguno de los dos — vale la pena que quede como decisión explícita (¿se construye la lista de espera real, o se corrige la documentación para sacar la mención?) en vez de seguir arrastrándose.
